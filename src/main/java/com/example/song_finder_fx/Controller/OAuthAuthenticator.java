...
public abstract class OAuthAuthenticator {
            ...
            if (event.getSource() instanceof WebEngine) {
                ...
                if (location.contains("code") && location.startsWith(getRedirectUri())) {
                    ...
                    try {
                        ...
                        AlertBuilder.sendErrorAlert("Error", "Error Logging in with Google Services", e.toString());
                    }
                }
            }
        });
    ...
    private boolean notifyLoginViewCompleted() throws SQLException {
        if(gotData) {
            // LoginView.getInstance().completedOAuthLogin(this);
            // System.out.println("Access Token: " + getAccessToken());
            // String accountInfo = doGetAccountInfo(getAccessToken());
            // System.out.println("accountInfo = " + accountInfo);

            JSONObject userData = getJsonData();
            ...
            String username = name.toString().toLowerCase().replace(" ", "_");

            DatabasePostgres.createUserGoogle((String) id, (String) name, username);

            Main.userSession.setPrivilegesAndEmail(username);
            Main.userSession.saveSession(username);
